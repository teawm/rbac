...
public class AuditLog {
    ...
    public static void main(String[] args) {
        System.out.println("[Тестирование AuditLog.]\n");

        AuditLog auditLog = new AuditLog();

        System.out.println("[Тест 1] Добавление записи:");
        try {
            auditLog.log("CREATE", "admin", "User: stas", "Создан пользователь");
            auditLog.log("CREATE", "admin", "Role: Administrator", "Создана роль");
            System.out.println("[v] Записи добавлены");
            System.out.println("  > Количество записей: " + auditLog.count());
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        }
        System.out.println();

        System.out.println("[Тест 2] Получение всех записей:");
        try {
            List<AuditEntry> all = auditLog.getAll();
            System.out.println("[v] Получено записей: " + all.size());
        ...
        System.out.println("[Тест 3] Поиск по исполнителю:");
        try {
            List<AuditEntry> byAdmin = auditLog.getByPerformer("admin");
            System.out.println("[v] Найдено записей от admin: " + byAdmin.size());
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        }
        System.out.println();

        System.out.println("[Тест 4] Поиск по действию:");
        try {
            List<AuditEntry> creates = auditLog.getByAction("CREATE");
            System.out.println("[v] Найдено записей с действием CREATE: " + creates.size());
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        }
        System.out.println();

        System.out.println("[Тест 5] Получение последних записей:");
        try {
            auditLog.log("UPDATE", "admin", "User: stas", "Обновлен email");
            auditLog.log("DELETE", "admin", "Role: Viewer", "Удалена роль");
            auditLog.log("ASSIGN", "admin", "Role: Administrator to User: stas", "Назначена роль");

            List<AuditEntry> recent = auditLog.getRecent(3);
            System.out.println("[v] Последние 3 записи:");
        ...
        System.out.println("[Тест 6] Вывод лога:");
        try {
            auditLog.printLog();
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        ...
        System.out.println("[Тест 7] Статистика:");
        try {
        ...
        System.out.println("[Тест 8] Сохранение в файл:");
        try {
            auditLog.saveToFile("audit_log_test.txt");
            System.out.println("[v] Файл сохранён");
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        }
        System.out.println();

        System.out.println("[Тест 9] Загрузка из файла:");
        try {
            AuditLog newLog = new AuditLog();
            newLog.loadFromFile("audit_log_test.txt");
            System.out.println("[v] Загружено записей: " + newLog.count());
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        }
        System.out.println();

        System.out.println("[Тест 10] Очистка лога:");
        try {
            int before = auditLog.count();
            auditLog.clear();
            int after = auditLog.count();
            System.out.println("[v] Записей до очистки: " + before);
            System.out.println("[v] Записей после очистки: " + after);
        } catch (Exception e) {
            System.out.println("[x] Ошибка: " + e.getMessage());
        }
        System.out.println();
